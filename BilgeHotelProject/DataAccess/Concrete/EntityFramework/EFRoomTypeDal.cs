...
namespace DataAccess.Concrete.EntityFramework
{
    ...
    {        
        ...
        public List<RoomType> AvaibleRoomTypes(DateTime checkinDate, DateTime checkoutDate, int numberOfPeople)
        {
            ...
            var roomTypes = db.StatusesOfRooms.Where(x => ((x.StatusStartDate! <= checkinDate && x.StatusEndDate! >= checkinDate) || (x.StatusStartDate! <= checkoutDate && x.StatusEndDate! >= checkoutDate)) && x.RoomStatus.StatusName != "Dolu").Select(x => x.Room.RoomType);
